...
func _rpc_announce_game_version(version: String):
...
func _communicate_disconnect_reason(id: int, reason: String):
	if multiplayer.is_server() or multiplayer.get_unique_id() != id:
		return
	_close_with_error(reason)

func _player_disconnected(id: int, sync_requested: bool = true):
...
func announce_name(data: Dictionary):
	...
	if _players.any(func(player: PlayerResource): return player.id == id):
		return
...
func _peer_id_is_connected(id: int):
	for current_peer in multiplayer.get_peers():
		if current_peer == id:
			return true
...
@rpc("authority", "reliable")
func rpc_sync_player_list(data: Dictionary):
	print(data)
	if multiplayer.is_server():
		return
	var players: Array[Dictionary] = data.get_or_add("players", [])
	print(players)
	if players.size() > 0:
		_players.clear()
		clear_players.emit()
	for player in players:
		var new_player := PlayerResource.from_network_data(player)
...
@rpc("authority", "reliable")
func _rpc_enforce_disconnect(id: int):
	if id != multiplayer.get_unique_id():
		return
	multiplayer.multiplayer_peer.close()
