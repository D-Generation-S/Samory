...
func _ready() -> void:
	for child: Node in get_tree().get_nodes_in_group(animation_scene_group):
		child.queue_free()
...
func change_screen_to(scene: PackedScene, old_texture: Texture, transition_start_position: Vector2) -> void:
	if scene == null:
		return
	var loaded_scene: Node = scene.instantiate()
	change_screen_to_node(loaded_scene, old_texture, transition_start_position)

func change_screen_to_node(node: Node, old_texture: Texture, transition_start_position: Vector2) -> void:
...
func _transition_animation(value: float) -> void:
	transition_state.emit(value)

func _transit_now(_step_id: int) -> void:
	if target_scene != null:
		for scene: Node in get_tree().get_nodes_in_group(active_scene_group):
			scene.queue_free()
