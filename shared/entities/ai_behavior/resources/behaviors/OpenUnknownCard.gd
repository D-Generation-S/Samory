...
func can_execute(blackboard: Blackboard, grid: GameCardGrid) -> bool:
	var unknown_card_positions: Array[Point] = get_all_unknown_card_positions(blackboard, grid)
...
func get_all_unknown_card_positions(blackboard: Blackboard, grid: GameCardGrid) -> Array[Point]:
	...
	for current_position: Point in all_positions:
		if known_positions.any(func(known_position: CardInformationResource) -> bool: return known_position.position.is_identical(current_position)):
			continue
...
func execute_action(blackboard: Blackboard, grid: GameCardGrid) -> void:
	...
	var cards: Array[Point]  = get_all_unknown_card_positions(blackboard, grid)
