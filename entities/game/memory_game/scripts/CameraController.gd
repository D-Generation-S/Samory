...
func handle_camera_movement() -> void:
	var zoom_vector: float = Input.get_axis("zoom_in", "zoom_out")
	...
	var zoom_x: float = clampf(zoom.x, min_zoom, max_zoom)
	zoom = Vector2(zoom_x, zoom_x)
	
	var controller_drag_vector: Vector2 = Input.get_vector("drag_left", "drag_right", "drag_up", "drag_down")
	...
	if offset != Vector2.ZERO:
		var position_x: float = clamp(position.x + offset.x, -max_x_range, max_x_range)
		var position_y: float = clamp(position.y + offset.y, -max_y_range, max_y_range)
...
func drag_mouse() -> void:
	if _initial_drag:
		_initial_drag = false
		_last_mouse_pos = get_viewport().get_mouse_position()
	
	var delta: Vector2 = get_viewport().get_mouse_position() - _last_mouse_pos
	offset = offset - Vector2(delta.x * drag_speed, delta.y * drag_speed)

	_last_mouse_pos = get_viewport().get_mouse_position()

func drag_controller(delta: Vector2) -> void:
	if delta == Vector2.ZERO:
		return
	offset = offset + delta * controller_drag_speed

func adjust_zoom_and_position_to_play_area(area: Rect2) -> void:
	var larger_side: float = max(area.size.x, area.size.y * 1.3)
